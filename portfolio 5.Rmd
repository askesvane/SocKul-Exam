...
d %>% group_by(Session) %>% summarise(
  mean_GR = mean(GR_Time_Scale, na.rm = T)
)
...
pp_check(priormodel1.1, nsamples=100)
...
pp_check(postmodel1.1, nsamples=100)
...
pp_check(priormodel1.2, nsamples=100)
...
pp_check(postmodel1.2, nsamples=100)
...
pp_check(priormodel1.3, nsamples=100)
...
pp_check(postmodel1.3, nsamples=100)
...
# formula
formula2.1 <- bf(FR_Time_Scale ~ 0 + Session + (1+Session|ID) + (1|Picture))

get_prior(formula2.1, d, family = "beta")
...
pp_check(postmodel2.1, nsamples=100)
...
# formula
formula2.2 <- bf(FR_Time_Scale ~ 0 + Session:Diagnosis + (1+Session|ID) + (1|Picture))

get_prior(formula2.2, d, family = "beta")
...
pp_check(priormodel2.2, nsamples=100)
...
pp_check(postmodel2.2, nsamples=100)
...
#"Across groups, fixations on first own rating, relative to total time on scale will be shorter in session 2."
hypothesis(postmodel2.1, "Session1 > Session2")
plot(hypothesis(postmodel2.1, "Session1 > Session2"))

hypothesis(postmodel2.2, "Session1:Diagnosis1 > Session2:Diagnosis1")
hypothesis(postmodel2.2, "Session1:Diagnosis0 > Session2:Diagnosis0")

plot(hypothesis(postmodel2.2, "Session1:Diagnosis1 > Session2:Diagnosis1"))
plot(hypothesis(postmodel2.2, "Session1:Diagnosis0 > Session2:Diagnosis0"))


### Hypothesis testing ###

# "The ratio between fixation time spent on first rating and fixation time spent on the entire scale, can be predicted by session interacting with diagnosis. We expect Session2:Diagnosis1 having a higher fixation time on previous rating (relative to scale time), compared to Session2:Diagnosis0"
hypothesis(postmodel2.2, "Session2:Diagnosis1 > Session2:Diagnosis0 ")
plot(hypothesis(postmodel2.2, "Session2:Diagnosis1 > Session2:Diagnosis0 "))
...
# First model
formula3.1 <- bf(FaceTimeSek ~ 1 + Session + (1 + Session | ID) + (1 | Picture))

get_prior(formula3.1, d, family = lognormal())

prior3.1 <- c(
  prior(normal(1.1, 0.2), class = Intercept),
  prior(normal(0, 0.2), class = b),
...
pp_check(priormodel3.1, nsamples=50)
...
pp_check(postmodel3.1, nsamples=100)


# Second model
formula3.2 <- bf(FaceTimeSek ~ 0 + Session + Diagnosis + (1 + Session | ID) + (1 | Picture))

get_prior(formula3.2, d, family = lognormal())
...
pp_check(priormodel3.2, nsamples=50)
...
pp_check(postmodel3.2, nsamples=100)



### Summaries for Hypothesis 3 ###

# Summary for model 1
summary(postmodel3.1)
(exp(fixef(postmodel3.1)))/(1+exp(fixef(postmodel3.1)))

#Summary for model 2
summary(postmodel3.2)
(exp(fixef(postmodel3.2)))/(1+exp(fixef(postmodel3.2)))


## Model Compare
postmodel3.1 <- add_criterion(postmodel3.1, criterion = c("waic","loo"))
postmodel3.2 <- add_criterion(postmodel3.2, criterion = c("waic","loo"))

loo_compare(postmodel3.1, postmodel3.2)
loo_model_weights(postmodel3.1, postmodel3.2)
...
hypothesis(postmodel3.1, "0 > Session2")
plot(hypothesis(postmodel3.1, "0 > Session2"))

# "Diagnosis SCZ will have lower FaceTimes than TD's for Session 1, and Session 2, respectively"
hypothesis(postmodel3.2, "Diagnosis1 < 0")
plot(hypothesis(postmodel3.2, "Diagnosis1 < 0"))


### Effects visualized ###
conditional_effects(postmodel3.1)
conditional_effects(postmodel3.2)
...
### Preprocess data for running re-analysis (H4) - Extract Social conformity scores
...
# Formula 1
formula4.1 <- bf(FaceTime2_sek ~ 1 + SocialConformityM + (1|ID) + (1|Picture) ) # + (1|Picture)

get_prior(formula4.1, d4, family = lognormal())
prior4.1 <- c(
  prior(normal(1.1, 0.2), class = Intercept),
  prior(normal(0, 0.2), class = b),
  prior(normal(0, 0.3), class = sd),
  prior(normal(0, 0.20), class = sigma))
...
pp_check(priormodel4.1, nsamples=80)
...
pp_check(postmodel4.1, nsamples=80)
summary(postmodel4.1)



# Formula 2
formula4.2 <- bf(FaceTime2_sek ~ 1 + SocialConformityM + Diagnosis + (1|ID) + (1|Picture)) 

get_prior(formula4.2, d4, family = lognormal())
prior4.2 <- c(
  prior(normal(1.1, 0.2), class = Intercept),
  prior(normal(0, 0.2), class = b),
  prior(normal(0, 0.3), class = sd),
...
pp_check(priormodel4.2, nsamples=80)
...
pp_check(postmodel4.2, nsamples=100)
summary(postmodel4.2)



### Run comparison ###
postmodel4.1 <- add_criterion(postmodel4.1,criterion="loo")
postmodel4.2 <- add_criterion(postmodel4.2,criterion="loo")

loo_compare(postmodel4.1,postmodel4.2)
loo_model_weights(postmodel4.1,postmodel4.2)


### Hypothesis testing ###
hypothesis(postmodel4.1,"SocialConformityM < 0")
plot(hypothesis(postmodel4.1,"SocialConformityM < 0"))

hypothesis(postmodel4.2,"SocialConformityM < 0")
plot(hypothesis(postmodel4.2,"SocialConformityM < 0"))

hypothesis(postmodel4.2,"Diagnosis1 > 0")
plot(hypothesis(postmodel4.2,"SocialConformityM < 0"))


# conditional effects
plot(conditional_effects(postmodel4.2), points=T, rug=T)[[1]] + theme_classic()
